  ...
  return (
    <>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid var(--surface-border)', color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                      ...
                      <th style={{ padding: '0.6rem 0.75rem', textAlign: 'left' }}>Cabang</th>
                  ...
                  <tbody>
                    {detailModal.data.map((row, i) => (
                      <tr key={row.id || i} style={{ borderBottom: '1px solid var(--surface-border)' }}>
                        ...
                        <td style={{ padding: '0.65rem 0.75rem', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{row.profiles?.branches?.name || '-'}</td>
      ...
      {activeTab === 'report' ? (
        <>
          {/* Summary Cards */}
          <div className="stat-grid responsive-grid-4" style={{ marginBottom: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
            ...
            <div className="stat-card" onClick={() => openDetailModal('terlambat_istirahat', 'Telat Istirahat')} style={{ cursor: 'pointer', transition: 'transform 0.15s, box-shadow 0.15s' }}
              ...
              <div className="stat-icon" style={{ color: '#6366f1', background: 'rgba(99,102,241,0.1)' }}><Calendar size={24} /></div>
            </div>
