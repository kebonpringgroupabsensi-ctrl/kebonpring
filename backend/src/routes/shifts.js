...
router.post('/', authorize('super_admin'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: err.message || 'Gagal membuat shift baru.' });
  }
});
...
router.put('/:id', authorize('super_admin'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: err.message || 'Gagal mengupdate shift.' });
  }
});
...
router.delete('/:id', authorize('super_admin'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Gagal menghapus shift.' });
  }
});
 ...
 */
router.post('/assignments/bulk', authorize('super_admin'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Gagal menyimpan jadwal shift.' });
  }
});
 ...
 */
router.delete('/assignments/bulk', authorize('super_admin'), async (req, res) => {
  try {
