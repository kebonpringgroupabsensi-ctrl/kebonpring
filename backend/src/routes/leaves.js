...
router.post('/', async (req, res) => {
  try {
    const { leave_type, start_date, end_date, reason, attachment_url, latitude, longitude } = req.body;
 ...
 */
router.put('/:id/review', authorize('admin_cabang', 'super_admin'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Gagal memproses izin.' });
  }
});
